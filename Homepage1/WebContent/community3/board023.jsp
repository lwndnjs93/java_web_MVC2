...
<!-- 공지글 처리 시작 -->
		...
		for(int i=0;i<boardList.size();i++){
			BoardBean3 bl=(BoardBean3)boardList.get(i);
		%>
		
		<tr>
         <td class="no"><%=bl.getBOARD3_NUM()%></td>          
         <input type='hidden' name="comment_cnt[]" id="comment_cnt199" value="1"></td>
         <td class="subject"> 
         <a href="./BoardDetailAction3.bo3?num=<%=bl.getBOARD3_NUM()%>">
         <%=bl.getBOARD3_SUBJECT()%>
         </a>
         
         </td>
         <td class="author"><%=bl.getBOARD3_NAME() %></td>         
         <td class="regDate"><%=bl.getBOARD3_DATE() %></td>         
         <td class="hitCount"><%=bl.getBOARD3_READCOUNT() %></td>                        
         </tr>
...
<div class="btnRight"><a href="./BoardWrite3.bo3"><img src="../Image/board_wirtebtn.gif" /></a></div>

<tr align=center height=20>
		<td colspan=7 style=font-family:Tahoma;font-size:10pt;>
			<%if(nowpage<=1){ %> [이전]&nbsp;
			<%}else{ %>
			<a href="./BoardList3.bo3?page=<%=nowpage-1 %>">[이전]</a>&nbsp;
			<%} %>
			...
			<%for(int a=startpage;a<=endpage;a++){
				...
				[<%=a %>]
				<%}else{ %>
				<a href="./BoardList3.bo3?page=<%=a %>">[<%=a %>]</a>&nbsp;
				<%} %>
			<%} %>
			...
			[다음]
			<%}else{ %>
			<a href="./BoardList3.bo3?page=<%=nowpage+1 %>">[다음]</a>
			<%} %>
		</td>
	</tr>       
